...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        register(state, user) {
            if (user.username == "" || user.password == "") {
                toast.show({ title: 'invalid username or password.', position: 'topright', type: 'error' });
                return;
            }
            getDocs(query(collection(db, 'users'), where('username', '==', user.username))).then(snapshot => {
                if (!snapshot.empty) {
                    toast.show({ title: 'user already exist.', position: 'topright', type: 'info' });
                    return;
                }
                addDoc(collection(db, 'users'), { ...user, img: null, createdAt: new Date() });
                toast.show({ title: 'user created.', position: 'topright', type: 'info' });
            });
        },
        setUser(state, user) {
            state.user = user;
            if (user) 
                ...
                localStorage.removeItem('manxUser');
        },
    ...
    actions: {
        login({ commit }, user) {
            const loginQuery = query(collection(db, 'users'), where('username', '==', user.username), where('password', '==', user.password));
            getDocs(loginQuery).then(snapshot => {
                if (snapshot.empty) {
                    toast.show({ title: 'user or password invalid.', position: 'topright', type: 'warn' });
                    return;
                } else {
                    let userId;
                    snapshot.forEach(doc => { userId = doc.id; commit('setUser', { id: doc.id, ...doc.data() }) });
                    const q = query(
